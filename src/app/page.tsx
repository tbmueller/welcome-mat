    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 px-4">
      ...
      <div className="flex flex-col items-center gap-3">
        <button
          onClick={signIn}
          disabled={signingIn}
          className="flex items-center gap-3 rounded-lg border border-taupe-300 bg-white px-6 py-3 text-sm font-medium shadow-sm transition hover:bg-taupe-50 disabled:opacity-60 disabled:cursor-not-allowed dark:border-taupe-600 dark:bg-taupe-800 dark:text-taupe-100 dark:hover:bg-taupe-700"
        >
          {signingIn ? (
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-taupe-400 border-t-transparent" />
          ) : (
        ...
        {signingIn && (
          <p className="text-xs text-[var(--gray-11)]">
            If a popup didn&apos;t open, you&apos;ll be redirected to Google instead.
          </p>
        )}
      </div>
    </div>
